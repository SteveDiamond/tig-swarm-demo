...
export class LeaderboardPanel implements Panel {
  ...
  private render() {
    ...
    sorted.forEach((entry, i) => {
      ...
      const scoreText = entry.best_score === null ? "—" : entry.best_score.toFixed(4);
      ...
      row.innerHTML = `
        ...
        <span class="lb-stag${entry.runs_since_improvement >= 3 ? " lb-stag--alert" : ""}">${entry.runs_since_improvement}</span>
